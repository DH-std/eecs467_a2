...
public:
    ...
    a2_blob_detector detector1;
    a2_blob_detector detector2;
    a2_image_to_arm_coord im2arm;
...
public:
	state_t(){
	    ...
	    im2arm = a2_image_to_arm_coord(im1,Hmin,Hmax,Smin,Smax,Vmin,Vmax);
	...
	void handleStatus(const lcm::ReceiveBuffer *rbuf,
	                  ...
	                  const dynamixel_status_list_t *msg)
		{
            ...
            for (int id = 0; id < msg->len; id++) {
                servo_pos[id] = msg->statuses.position_radians[id];
    ...
    void tttHandler(const lcm::ReceiveBuffer *rbuf,
		...
		{
			//check if our turn
			if(((red == true) && (msg.turn_num%2 == 0)) or ((red == false) && (msg.turn_num%2 == 1)) ){
